...
# @registry.register_task('remote_homology', num_labels=7)
...
class RemoteHomologyDataset(Dataset):
    ...
    def __init__(self,
        ...
        if split not in ('train', 'valid', 'test_fold_holdout', 'test_fold_holdout_hard',
        ...
        data_path = Path(data_path)
        data_file = f'remote_homology/remote_homology_{split}.json'
        self.data = dataset_factory(data_path / data_file, in_memory)

        with open("seq_to_deepsf_features.pickle", "rb") as handle:
            self.seq_to_deepsf_features = pkl.load(handle, encoding='latin1')
    ...
    def __getitem__(self, index: int):
        ...
        token_ids = self.tokenizer.encode(item['primary'])
        input_mask = np.ones_like(token_ids)
        deep_sf = self.seq_to_deepsf_features[item['primary']]
        # return token_ids, input_mask, item['fold_label']
        return token_ids, deep_sf, input_mask, item['fold_label']
        # return token_ids, deep_sf, input_mask, item['class_label']

    def collate_fn(self, batch: List[Tuple[Any, ...]]) -> Dict[str, torch.Tensor]:
        # input_ids, input_mask, fold_label = tuple(zip(*batch))
        input_ids, deep_sf, input_mask, fold_label = tuple(zip(*batch))
        input_ids = torch.from_numpy(pad_sequences(input_ids, 0))
        deep_sf = torch.tensor(deep_sf, dtype=torch.float32)
        input_mask = torch.from_numpy(pad_sequences(input_mask, 0))
...
class ProteinnetDataset(Dataset):
    ...
    def __init__(self,
        ...
        data_path = Path(data_path)
        data_file = f'proteinnet/proteinnet_{split}.lmbd'
        self.data = dataset_factory(data_path / data_file, in_memory)
