...
class ProteinBertForSequenceClassification(ProteinBertAbstractModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        # print("using BERT with DeepSF features")
        ...
        self.lin2 = nn.Linear(300, config.num_labels)
        self.relu = nn.ReLU()
        self.classify = SequenceClassificationHead(
            # config.hidden_size, config.num_labels)
            config.num_labels * 2, config.num_labels)
            # config.hidden_size + 300, config.num_labels)

        self.init_weights()

    # def forward(self, input_ids, input_mask=None, targets=None):
    def forward(self, input_ids, deep_sf, input_mask=None, targets=None):

        outputs = self.bert(input_ids, input_mask=input_mask)

        sequence_output, pooled_output = outputs[:2]
        # deep_sf_out = self.relu(self.lin1(deep_sf))
        # pooled_output = torch.cat((pooled_output, deep_sf_out), dim=1)
        # pooled_output = self.dropout(pooled_output)

        pooled_output = self.relu(self.lin1(pooled_output))
        deep_sf_out = self.relu(self.lin2(deep_sf))
        pooled_output = torch.cat((pooled_output, deep_sf_out), dim=1)
