...
def run_eval_epoch(eval_loader: DataLoader,
    ...
    data_dict = {entry["primary"]: entry["id"] for entry in eval_loader.dataset.data}
    ...
    for batch in tqdm(eval_loader, desc='Evaluation', total=len(eval_loader),
        ...
        for k in range(batch['input_ids'].shape[0]):
            seq = "".join([c for c in tokenizer.convert_ids_to_tokens(batch['input_ids'][k]) if c != "<pad>"][1:-1])
            seqs.append(seq)
            ids.append(data_dict[seq])
        for pred, target, seqs, ids in zip(predictions, targets, seqs, ids):
            save_outputs.append({'prediction': pred, 'target': target, 'seq': seqs, 'ids': ids})
...
def run_train(model_type: str,
    ...
    logger.info("  Num of parameters = %d", num_trainable_parameters)
